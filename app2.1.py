...
def play_sound_async(sound_type):
    ...
    def _play():
        ...
        # Windows系统（包括win32/win64/win10+/win11）使用系统音频播放器
            ...
            try:
                import winsound
                # Windows系统尝试使用winsound播放音频文件
                if sound_type == "beep":
                    # 进入长休息音效
                    try:
                        winsound.PlaySound(os.path.join(SCRIPT_DIR, 'long_rest_start.mp3'), winsound.SND_FILENAME)
                    except:
                        winsound.Beep(800, 300)  # 频率800Hz，持续300毫秒
                elif sound_type == "start_focus":
                    # 开始专注的音效
                    try:
                        winsound.PlaySound(os.path.join(SCRIPT_DIR, 'short_rest_end.mp3'), winsound.SND_FILENAME)
                    except:
                        winsound.Beep(1000, 400)  # 频率1000Hz，持续400毫秒
                elif sound_type == "rest_start":
                    # 进入短休息的音效 - Windows使用.mp3格式
                    try:
                        winsound.PlaySound(os.path.join(SCRIPT_DIR, 'short_rest_start.mp3'), winsound.SND_FILENAME)
                    except:
